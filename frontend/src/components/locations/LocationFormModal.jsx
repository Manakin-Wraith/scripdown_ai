...
/**
 * Controlled create/edit form for a location, rendered as a modal.
 * Props: { initial, contacts, onSubmit, onClose, saving, title }
 *
 * lat/lng are only included in the payload when the user set or changed them
 * (via the Locate button or by editing the fields), so the backend's
 * manual-vs-geocode branch fires correctly.
 */
export default function LocationFormModal({ initial, contacts = [], onSubmit, onClose, saving, title }) {
    ...
    return (
        <div className="production-modal-backdrop" onClick={onClose}>
            <div className="production-modal" onClick={(e) => e.stopPropagation()}>
                <h3>{title || (isEdit ? 'Edit location' : 'New location')}</h3>
