...
- **CSV/import-driven template creation** — today a template is only built by hand, field-by-field, in the Call Sheet tab; a production coming from an existing house-style call sheet (their own CSV/spreadsheet layout, or an export from another tool) has no way to bring it in directly. Needs a brainstorm (see `superpowers:brainstorming`) — open questions: what "upload a CSV template" actually means here (a column-header row mapped to day fields/custom columns, vs. a full section/layout description, since a flat CSV can't naturally express the shipped config's nested shape — sections, per-column types, sensitive flags, blocks); how much of the shipped `validate_config` schema (Task 3: key regex, type set, caps, locked builtins) a CSV import needs to map into or reject; whether this replaces manual building for that production or seeds it (import once, then edit in the existing tab); and how errors in a malformed CSV surface (the shipped template PUT's `{path, message}` validation-error shape is a natural fit to reuse).
