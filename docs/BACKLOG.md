...
- **CSV/import-driven template creation — brainstorm in progress, paused 2026-09-23 (superpowers:brainstorming, architectural path).** Today a template is only built by hand, field-by-field, in the Call Sheet tab; this lets an admin bring in a bunch of fields/columns/departments/blocks at once from a CSV instead. Decisions made so far, to resume from directly without re-deriving them:
  - **Structure only for v1** — no seeding of real per-day values from the CSV (that's a real, separately-scoped follow-up: which day gets the values, a day-picker, a second CSV shape with a `value` column, merge into `update_call_sheet_with_report` — deliberately deferred, not designed).
  - **No AI.** Deterministic parsing only, matching the existing `crew_import.py` precedent exactly: a strict, documented CSV column convention (not an arbitrary/heuristically-detected layout) — a `kind` column (`day_field` / `cast_column` / `crew_column` / `scene_column` / `department` / `block` / `key_crew`) discriminates which config list each row belongs to, remaining columns map to that item's schema fields (key, label, type, section, sensitive, etc. — exact column set per kind still needs pinning down in the spec).
  - **Sections are out of scope for the CSV** — visibility/order/labels for the 11 sections always default to the standard set after an import; adjusting them stays a manual-builder-only action (11 checkboxes, not worth a CSV row shape).
  - **Existing template handling: replace, not merge** — a CSV import replaces the whole template wholesale (simplest mental model: "this file IS my template now"). Exception forced by the schema itself: **builtins can never be wiped** (`validate_config` requires all 10 builtin day fields to always exist) — replace specifically means start from `default_config()`'s builtins, apply any CSV `day_field` row whose key matches a builtin as an edit to it (label/section/visible/default — type stays locked, same rule the manual builder already enforces), and any custom (non-builtin) day field/column/department/block/key-crew entry from the *previous* template not present in the new CSV is dropped.
  - **Preview before commit is mandatory** (this is the one place it diverges from crew import's immediate-commit-then-summary UX) — and it's a **rendered mock of the actual call sheet**, not just a rows table: reuses `call_sheet_render.render_html` completely unchanged, fed the candidate (unsaved) config plus fabricated sample data, with parse errors/skipped rows shown as a banner above it (same info shape as crew import's post-import summary, just shown before commit).
  - **Near-zero new backend surface, by design.** The parser is a new pure module (`backend/services/call_sheet_template_import.py`, no DB access, mirrors `crew_import.py`'s shape) that builds a candidate config; that candidate runs through the *already-shipped* `validate_config` (Task 3) with no new validation logic. One new endpoint — `POST /api/productions/<id>/call-sheet-template/import-preview` — parses + validates + renders and **saves nothing**; the review screen's "Confirm" button calls the *already-existing* `saveCallSheetTemplate` (Task 10) with the returned config. No new save path at all.
  - **Depends on / shares a component with** the "Live/rendered preview in the template builder" item directly above — same `render_html`-based preview call, two entry points (manual edits, CSV import). Worth building together or at least in the same brainstorm/spec pass.
  - **Still open when this resumes:** exact CSV column names/shape per `kind` (not yet pinned down — was left for the spec doc); fabricated sample-data shape for the preview render (reuse the Task 9 visual-check fixture as a starting point); frontend upload/review-screen component design; whether a downloadable starter-template CSV (mirroring crew import's `/crew-import-template.csv`) should be pre-filled from the production's *current* template config (a natural companion "export template to CSV" feature) or a static blank; test plan for the new parser module; whether file-size/row-count caps beyond the schema's existing caps (40 day fields, 15 columns, etc.) are needed for the upload itself.
