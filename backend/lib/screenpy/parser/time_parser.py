...
- Loads time words from locale_config (config-driven)
...
from ..locale_config import get_locale, ScreenplayLocale
...
class TimeParser:
    ...
    - Locale-aware times (DAG, NAG for Afrikaans, JOUR/NUIT for French)
    ...
    def __init__(self, locale_codes: list[str] | None = None):
        """
        Initialize with locale-aware time words.

        Args:
            locale_codes: Locale code(s) to load. Defaults to ["en"].
        """
        self.locale = get_locale(locale_codes)
        self._init_time_words()
        self._init_patterns()

    def _init_time_words(self):
        """Initialize sets of time-related words from locale."""
        self.times_of_day = set(self.locale.times_of_day)
        self.relative_times = set(self.locale.relative_times)
        self.seasons = set(self.locale.seasons)
        self.weather_time = set(self.locale.weather_time)

        # Weekdays and months are language-neutral enough to keep English
        self.weekdays = {
        ...
        # Holidays (mostly English — locale-specific holidays could be added)
        self.holidays = {
